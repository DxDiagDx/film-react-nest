...
export class OrderService {
    ...
    async createOrder(createOrderDto: CreateOrderDto): Promise<{ success: boolean; message: string; order?: any }> {
        ...
        if (conflictingSeats.length > 0) {
            throw new BadRequestException(`Места ${conflictingSeats.join(', ')} уже заняты`);
        }

        // 7. Обновляем занятые места в фильме
        await (this.filmsRepository as any).updateTakenSeats?.(filmId, scheduleId, seats);
    ...
    private validateSeatsFormat(seats: string[]): void {
        ...
        if (invalidSeats.length > 0) {
            throw new BadRequestException(`Неверный формат места: ${invalidSeats.join(', ')}. Используйте формат: "row:seat"`);
        }
    }

    private validateSeatsExistence(seats: string[], maxRows: number, maxSeats: number): void {
        ...
        seats.forEach(seat => {
            ...
            if (invalidSeats.length > 0) {
                throw new BadRequestException(`Такого места не существует: ${invalidSeats.join(', ')}`);
            }
        })
    }

    private findConflictingSeats(requestedSeats: string[], takenSeats: string[]): string[] {
